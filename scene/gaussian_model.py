...
class GaussianModel:
    ...
    def add_point_using_closest_colmap(self, new_xyz, radii):
        ...
        closest_tmp_radius = 1.0  # 如果存在，就给一个默认值

        # 6. 将新点和从最近 COLMAP 点提取的特征传递给 densification_postfix
        self.densification_postfix(
            new_xyz=new_xyz.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
            new_features_dc=closest_features_dc.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
            new_features_rest=closest_features_rest.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
            new_opacities=closest_opacity.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
            new_scaling=closest_scaling.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
            new_rotation=closest_rotation.unsqueeze(0).to('cuda'),  # 在第一个维度加上维度 1
